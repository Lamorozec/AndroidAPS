...
class CarelevoUpdateMaxBolusDoseUseCase @Inject constructor(
    ...
    /** True if an immediate or extended bolus is in progress — a setting must not be pushed mid-bolus (mirrors case1). */
    ...
    /**
     * Persist the max-bolus setting for the Phase-2 new-BLE-stack path. [synced] = the patch confirmed the
     * push; when false, `needMaxBolusDoseSyncPatch` stays true so the deferred-sync re-pushes on the next
     * reconnect (mirrors the legacy fail/disconnected cases). Returns false with no setting record or on a
     * failed write.
     */
