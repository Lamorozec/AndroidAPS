 ...
 *
 * Request wire format (1 byte): `[0] 0x1D`. Legacy `manipulateAdditionalPriming` builds the message from the
 * opcode alone with no argument transformers, so there is nothing to encode or range-check.
 *
 * Response (0x7D): `[0] 0x7D, [1] resultCode` → [SimpleResultResponse] (`resultCode` 0 = SUCCESS). Mirrors
 * `CarelevoProtocolAdditionalPrimingParserImpl`, which reads only `data[0]` (cmd) and `data[1]` (result).
 *
