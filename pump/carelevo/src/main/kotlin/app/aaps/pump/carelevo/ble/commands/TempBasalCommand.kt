 ...
 * delivery**. One command class for both legacy consumers, parameterized by [Mode]:
 * - [Mode.BY_UNIT]    — legacy `manipulateStartTempBasalInfusionProgramByUnit`, an absolute rate in U/h.
 * - [Mode.BY_PERCENT] — legacy `manipulateStartTempBasalInfusionProgramByPercent`, a percentage of the
 *   scheduled basal.
 *
 * **Reproduce the ByUnit↔ByPercent ASYMMETRY exactly** (a wrong byte here is a patient-safety bug):
 * ```
 ...
 * ```
 * Quirks reproduced byte-for-byte from the legacy transformer chain:
 * - **ByUnit trailing 0x00:** legacy appends `BooleanToByte(true)`, and `BooleanToByte` maps
 *   `true→0x00` — so the sixth byte is `0x00`. ByPercent appends **nothing** (the asymmetry).
 * - **ByPercent `/100.0`:** legacy encodes `infusionPercent.toDouble() / 100.0` (e.g. 150 % → `1.50` →
 *   `[1, 50]`); [byPercent] applies the same division before [encodeUnitCenti].
 * - **ByUnit has NO value-range check:** legacy `DoubleToByte(0.0, 0.0)` short-circuits its own range
 *   guard (min==max==0 ⇒ bypass), so any `infusionUnit` is accepted. ByPercent uses
 *   `DoubleToByte(0.0, 200.0)` on the divided value, so `percent/100.0` must be in `0.0..200.0`.
 * - hour is `IntegerToByte(0, 24)`, minute is `IntegerToByte(0, 59)` — same ranges for both modes.
 *
 * Response (0x83): `[0] 0x83, [1] resultCode` → [SimpleResultResponse] (0 = SUCCESS). The legacy parser
 * also fabricates a local `System.currentTimeMillis()` timestamp — that is NOT on the wire and is dropped
 * here (the wire reply is result-only).
 */
class TempBasalCommand private constructor(
    ...
    /** Which of the two legacy 0x23 variants this instance encodes. Use [byUnit] / [byPercent] to build. */
    ...
    init {
        ...
        // BY_UNIT deliberately has no value-range check — legacy DoubleToByte(0.0, 0.0) bypasses its guard.
    ...
    override fun encode(): ByteArray {
        ...
        return when (mode) {
            Mode.BY_UNIT    -> head + byteArrayOf(TRAILING_BOOL_TRUE) // legacy BooleanToByte(true) = 0x00
    ...
    companion object {
        ...
        private const val TRAILING_BOOL_TRUE: Byte = 0x00 // BooleanToByte(true) = 0x00
         ...
         * No range check on [infusionUnit] (legacy quirk). Encodes the 6-byte frame with the trailing 0x00.
         */
