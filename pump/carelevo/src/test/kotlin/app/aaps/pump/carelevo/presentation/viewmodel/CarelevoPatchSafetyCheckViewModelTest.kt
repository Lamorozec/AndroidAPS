 ...
 * `Dispatchers.Main.immediate`. Installing an [UnconfinedTestDispatcher] as Main therefore makes the
 * whole VM testable on a plain JVM, and makes every `viewModelScope.launch` (the event emits, the
...
internal class CarelevoPatchSafetyCheckViewModelTest {
    ...
    fun setUp() {
        ...
        whenever(activationExecutor.safetyProgress).thenReturn(MutableSharedFlow<SafetyProgress>())
    ...
    fun `a safety-check progress frame seeds the countdown before the terminal success`() {
        // A StateFlow replays its value on subscribe, so the VM's progressJob picks the Progress frame
        // up synchronously (Unconfined) before customCommand returns — exercising the ticker branch.
        whenever(carelevoPatch.isBluetoothEnabled()).thenReturn(true)
        whenever(activationExecutor.safetyProgress).thenReturn(MutableStateFlow<SafetyProgress>(SafetyProgress.Progress(60L)))
        stubCustomCommand(enactResult(true))
        val events = collectEvents()
        val progressValues = collectProgress()
        val remainValues = collectRemainSec()

        sut.startSafetyCheck()

        // Progress frame → bar reset to 0 and the countdown seeded with the frame's timeout...
        assertThat(progressValues).contains(0)
        assertThat(remainValues).contains(60L)
